...
```{r echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, cache = TRUE, cache.path = 'cache/', fig.path = 'figure/', results = "markup", BoxPlot}

# Put comments in a list for the plots. 
fixed.acidity.comment <- "The mean and median of the fixed.acidity feature does not appear to change much with quality. Additionally, the range of the middle 50% of the data is largely different only for the quaity level of 9."

volatile.acidity.comment <- "It is not entirely clear if volatile.acidity affects quality. There is a minimum median at the quality level of 6. The range of the middle 50% of the data seems to increase with quality."

citric.acid.comment <- "It is not entirely clear if citric.acid affects quality. The mean and median increases at the highest quality, and the range decreases with increasing quality."

residual.sugar.comment <- "It is not entirely clear if residual.sugar affects quality. There is a maximum median at the quality level of 5 and there is a minimum median at the quality level of 9."

chlorides.comment <- "The mean and median of the chlorides feature appears to decrease with quality. Additionally, the range of the middle 50% of the data gets smaller with quality."
  
free.sulfur.dioxide.comment <- "It is not entirely clear if free.sulfur.dioxide affects quality. The mean is lower for quality levels 3 - 4 than for levels 5 - 9. The mean/median does not change much for levels 5 - 9."
  
total.sulfur.dioxide.comment <- "The mean and median of the total.sulfur.dioxide feature appears to decrease with quality. Additionally, the range of the middle 50% of the data gets smaller with quality."

density.comment <- "The mean and median of the density feature appears to decrease with quality. Additionally, the range of the middle 50% of the data gets smaller with quality."
  
pH.comment <- "The mean and median of the pH feature appears to increase with quality. Additionally, the range of the middle 50% of the data gets smaller with quality."
  
sulphates.comment <- "The mean and median of the sulphates feature does not appear to change much with quality."
  
alcohol.comment <- "The mean and median of the alcohol feature appears to increase with quality. Additionally, the range of the middle 50% of the data gets smaller with quality."

# Put comments in a list  
box_comments <- c(fixed.acidity.comment, volatile.acidity.comment,
                  citric.acid.comment, residual.sugar.comment, 
                  chlorides.comment, free.sulfur.dioxide.comment,
                  total.sulfur.dioxide.comment, density.comment, pH.comment,
                  sulphates.comment, alcohol.comment)

# The indices of the comments list are features in the wine dataframe
names(box_comments) <- names(wine)[2:12]
...
for (i in names(wine)[2:12]) {
        p <- ggplot(subset(wine, !is.na(quality) & !is.na(i)), 
...
```{r echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, cache = TRUE, cache.path = 'cache/', fig.path = 'figure/', Scatterplot_1a}
...
```{r echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, cache = TRUE, cache.path = 'cache/', fig.path = 'figure/', Scatterplot_1b}
...
```{r echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.height = 2, results = "markup", cache = TRUE, cache.path = 'cache/', fig.path = 'figure/', QualityGroup}
...
# Make comments for the plots. 
volatile.acidity.comment <- "The correlation of the medians of volatile.acidity and quality is moderately strong. However, plot shows that there is not a strong trend in quality change due to volatile.acidity."

citric.acid.comment <- "The correlation of the medians of citric.acid and quality is moderately strong. However, plot shows that there is not a strong trend in quality change due to citric.acid."

residual.sugar.comment <- "The correlation of the medians of residual.sugar and quality is moderately strong. However, plot shows that there is not a strong trend in quality change due to residual.sugar."

chlorides.comment <- "The correlation of the medians of chlorides and quality is very strong. The plot also shows that there is a strong trend in quality change due to chlorides."
  
free.sulfur.dioxide.comment <- "The correlation of the medians of free.sulfur.dioxide and quality is weak. The plot also shows that there is a weak trend in quality change due to free.sulfur.dioxide."
  
total.sulfur.dioxide.comment <- "The correlation of the medians of total.sulfur.dioxide and quality is quite strong. The plot also shows that there is a strong trend in quality change due to total.sulfur.dioxide."

density.comment <- "The correlation of the medians of density and quality is very strong. The plot also shows that there is a strong trend in quality change due to density."
  
pH.comment <- "The correlation of the medians of pH and quality is quite strong. The plot also shows that there is a strong trend in quality change due to pH."
  
alcohol.comment <- "The correlation of the medians of alcohol and quality is very strong. The plot also shows that there is a strong trend in quality change due to alcohol."

# Put comments in a list  
group_comments <- c(volatile.acidity.comment, citric.acid.comment, 
                    residual.sugar.comment, chlorides.comment, 
                    free.sulfur.dioxide.comment, total.sulfur.dioxide.comment, 
                    density.comment, pH.comment, alcohol.comment)

# The indices of the comments list are features in the wine dataframe
names(group_comments) <- names(wine_by_quality)[2:10]

#lapply((names(wine_by_quality)[2:10]), 
for (i in names(wine_by_quality)[2:10]) {
  p <- ggplot(wine_by_quality, aes_string(x = i, y = "quality")) +
         geom_point() + 
         geom_smooth(method = "lm", color = "black")
  print(p)
  print(paste0("Correlation is ", cor(x = wine_by_quality[[i]], 
                                      y = wine_by_quality$quality, 
                                      method = "pearson")))
  print(group_comments[[i]])
}
...
if the wines in the data set should be separated by this feature. The following
two histograms show the two subsets of wine.
...
To see if a subset is different from the general wine population, the means and
correlations of the different features in the subset are compared to the global
means and correlations (presented in a previous section). The means of each
feature and a matrix plot for this subset is presented below.
...
common characteristics. To see if a subset is different from the general wine 
population, the means and correlations of the different features in the subset 
are compared to the global means and correlations (presented in a previous 
section). The means of each feature and a matrix plot for this subset is 
presented below.
...
became easier and I was able to ask interesting questions. 

I think finding the interesting subsets (citric acid peak, and residual sugar subsets) will be useful for future analysis and applying a machine learning algorithm.